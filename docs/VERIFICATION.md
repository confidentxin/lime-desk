...
- 三栏滚动条是否低干扰，不抢主视觉。
- 搜索结果、RAG 条目、选题、文案、配图方案与成品画廊区域是否被裁切、剪裁或遮挡。
- 搜索、RAG、选题空状态是否紧凑、居中、可读。
- 小红书预览与成品画廊是否保持稳定图片比例（预览封面 1:1、画廊 4:5）。
- 左侧保存入口、中间底部配图区、右侧错误覆盖是否能在各自栏内滚动访问。
- 右侧栏卡片是否被压扁或裁切（`.config-rail` 子项需要 `flex: none`，面板需要 `min-height`）。
- 文案是否可读，按钮文字是否完整。
- 文本、按钮、卡片是否互不重叠。
- 品牌标识（`lime-desk-mark.svg`）与配图是否正常加载。
- 品牌强调色是否只有青柠绿一个色系，未出现黏土时代的粉紫渐变或大面积铺色。
